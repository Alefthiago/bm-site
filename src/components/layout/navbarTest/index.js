...
import IndexesMobile from '../navbar/indexMobile';
//      /UTIL       //
...
const Component = (props) => {
    ...
    function verifyRoute(route, mobile = false) {
        if (route == pathname) {
            ...
            return `${mobile ? 'bg-[#206BA5] text-[#F8F8F8]' : ''}`;
        }
    }
    //      /FUNCOES     //
    ...
    return (
        <Navbar fluid className={`text-[#F8F8F8] bg-[#206BA5]`}>
            ...
            <Navbar.Collapse className={`md:block hidden`}>
                <Navbar.Link href={`/`} className={`${navbar_link}`}>Início</Navbar.Link>
                <Navbar.Link href={'/solutions'} className={`${navbar_link}`}>Soluções</Navbar.Link>
                <Navbar.Link href={'/equipment'} className={`${navbar_link}`}>Equipamentos</Navbar.Link>
                <Navbar.Link href={'/company'} className={`${navbar_link}`}>Empresa</Navbar.Link>
                <Navbar.Link href={'/contact'} className={`${navbar_link}`}>Contato</Navbar.Link>
                <Navbar.Link href={'/links'} className={`${navbar_link}`}>Links Úteis</Navbar.Link>
            </Navbar.Collapse>
        </Navbar>
    );
}
